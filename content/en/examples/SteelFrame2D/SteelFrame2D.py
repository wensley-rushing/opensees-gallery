...
def main():
    ...
    sections = {
        colSecTag1 : from_aisc("W10x26", mesh_scale=1/15, units=units),
        colSecTag2 : from_aisc("W10x26", mesh_scale=1/15, units=units),
        beamSecTag1: from_aisc( "W8x48", mesh_scale=1/15, units=units),
        beamSecTag2: from_aisc( "W8x48", mesh_scale=1/15, units=units),
        beamSecTag3: from_aisc( "W8x48", mesh_scale=1/15, units=units)
    }

    for tag, shape in sections.items():
        model.section("Fiber", tag)
        for fiber in shape.create_fibers(warp=False):
            model.fiber(fiber["z"], 0, area=fiber["area"], material=matTag, section=tag)
    ...
    model.element('ForceBeamColumn',  1, ( 1, 11), ColTransfTag, ColIntTag1)
    model.element('ForceBeamColumn',  2, ( 2, 12), ColTransfTag, ColIntTag2)
    model.element('ForceBeamColumn',  3, ( 3, 13), ColTransfTag, ColIntTag1)

    model.element('ForceBeamColumn', 11, (11, 21), ColTransfTag, ColIntTag1)
    model.element('ForceBeamColumn', 12, (12, 22), ColTransfTag, ColIntTag2)
    model.element('ForceBeamColumn', 13, (13, 23), ColTransfTag, ColIntTag1)

    model.element('ForceBeamColumn', 21, (21, 31), ColTransfTag, ColIntTag1)
    model.element('ForceBeamColumn', 22, (22, 32), ColTransfTag, ColIntTag2)
    model.element('ForceBeamColumn', 23, (23, 33), ColTransfTag, ColIntTag1)
    ...
    if AnalysisType == "Pushover":
        ...
        model.load(11, 1.61, 0.0, 0.0, pattern=2)
        model.load(21, 3.22, 0.0, 0.0, pattern=2)
        model.load(31, 4.83, 0.0, 0.0, pattern=2)
    ...
    #   model.recorder('Element', '-file', "PushoverOut/BeamStress.out", '-closeOnWrite', '-ele', 102, 'section', '4', 'fiber','1', 'stressStrain')
        ...
        for j in range(NstepsPush):
            if model.analyze(1) != ops.successful:
