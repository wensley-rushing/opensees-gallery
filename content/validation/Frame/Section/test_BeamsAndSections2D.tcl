...
proc printEigenvalues {E A I L} {
    ...
    for {set i 0} {$i < 3} {incr i} {
        verify value  [lindex $eigenValues [expr $i+3]]  [lindex $exact $i]
#       puts "  [lindex $eigenValues [expr $i+3]]  [lindex $exact $i]"
    }
}

proc printDisplacements {E A I L P H M} {
    verify value  [nodeDisp 2 1]  [expr $P*$L/($E*$A)]
    verify value  [nodeDisp 2 2]  [expr $H*pow($L,3)/(3*$E*$I) + $M*pow($L,2)/(2*$E*$I)]
    verify value  [nodeDisp 2 3]  [expr $H*pow($L,2)/(2*$E*$I) + $M*$L/($E*$I)]
}
